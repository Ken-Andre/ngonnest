...
## Complete User Story Flow: Sync Feature
...
sequenceDiagram
    ...
    participant SAPI as SupabaseApiService
    ...
    AS->>SAPI: Check for existing session
    SAPI-->>AS: Session status
    AS-->>A: Auth state
    ...
    Note over U,A: Enable Sync Process
    U->>A: Navigate to Settings
    ...
    Note over U,A: Using App with Sync Enabled
    ...
    SS->>SAPI: Send to Supabase
    SAPI->>SAPI: Process operation based on type
    alt For 'objet' entity
        SAPI->>SAPI: _syncProduct()
    else For 'foyer' entity
        SAPI->>SAPI: _syncHousehold()
    else For 'reachat_log' entity
        SAPI->>SAPI: _syncPurchase()
    else For 'budget_categories' entity
        SAPI->>SAPI: _syncBudgetCategory()
    end
    SAPI-->>SS: Response
